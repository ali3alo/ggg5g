...
@sython.on(events.NewMessage(outgoing=True, pattern=r"\.BanD User"))
async def _(event):
    if ispay2[0] == "yes":
...
@sython.on(events.NewMessage(outgoing=True, pattern=r"\.Fishing (.*)"))
async def _(event):
    if ispay2[0] == "yes":
        isclaim.clear()
        isclaim.append("on")
...
CH / @ggg5g
    ...
    {str(eee)}''')
                    if "A wait of" in str(eee):
                        ...
                        await sython.send_message(event.chat.id, " خاصيه او مبند  !")
            else:
                pass
            trys += 1
